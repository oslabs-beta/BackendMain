...
userController.getUserQueries = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
 ...
 try {
    ...
    if (user?.categories){
        ...
        return next({log: 'User id not found', message: {err: 'User id not found, please log in'}, status: 400})
    }
 }
 catch(error){
           const err = {
            log: 'Express error handler caught error in getUserQueries Middleware: ' + error,
            status: 500,
...
userController.addDataSource = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    if (user) {
      console.log("successfully added data source");
...
userController.addCategory = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
const { userId } = req.session;
const {category} = req.body;

try {
    const newCategory = await User.findByIdAndUpdate(userId, 
    { $set: { [`categories.${category}`]: []}},
    {new: true }
     );
    
    if (newCategory){
        res.locals.newCategory = newCategory.categories; return next()
    } else {
        return next({
          log: "User not found",
...
userController.addQueries = async (req: Request, res: Response, next: NextFunction
): Promise<void> => {
    const { category, query } = req.body;
    const { userId } = req.session;

    try {
        const userQuery = await User.findOneAndUpdate({_id: userId}, {
            $set: {[`categories.${category}`]: { $ifNull: [`$categories.${category}`, [] ] } },
            $push: {[`categories.${category}`]: query}
        }, { new: true} );
    ...
    const err = {
        log:
        "Express error handler caught error in addQueries Middleware: " +
        error,
        status: 500,
        message: { err: "user authentication failed!" },
    };
...
userController.deleteQuery = async (req: Request, res: Response, next: NextFunction
): Promise<void> => {
    const {userId} = req.session
    const {category, query} = req.body;

    try{
        const deleteQ = await User.findOneAndUpdate({_id: userId}, {
        ...
        if (deleteQ){
            ...
            return next( { log: `Unable to delete query `,
...
userController.deleteCategory = async (req: Request, res: Response, next: NextFunction
): Promise<void> => {
    const { category } = req.body;
    const { userId } = req.session;

    try {
        const deleteC = await User.findByIdAndUpdate( userId, {
